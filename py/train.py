...
def main(
    ...
    # name = 'movie_model0'
    # nn = build_model(input_shape, output_shape=n_movies, name=name)
    # nn = Sequential([
    ...
    #     Dense(n_movies, activation='softmax'),
    # ], name='test_model')
    # nn.compile(
    #     optimizer='adam',
    ...
    # return inputs_consumer, label_test, label_pred, conf_matrix, nn
...
def check_accuracy(
    ...
    predict_ids = []
    y_pred = []
    y_test_one_hot = np.empty((1, n_movies))
    ...
    for consumer, view_ids, preference in iter_check_accuracy:
        ...
        # 「何も見ていない」パターンを除外する
        viewed_movie = view_ids_unique != n_movies
        view_ids_one_hot = np.eye(period)[view_ids_unique][:, :n_movies]
        y_test_one_hot = np.concatenate(
            [y_test_one_hot, view_ids_one_hot[viewed_movie]],
            axis=0
        )
        iter_viewing = zip(
            view_ids_unique[viewed_movie],
            view_ids_one_hot[viewed_movie],
            datumn[viewed_movie, :-1]
        )
        # print('mispredicted:')
    ...
    print(f"{np.bincount(predict_ids)=}")
    print(f"{predict_ids_set=}")
    if len(predict_ids_set) == 1:
        pred_id = predict_ids[0]
        pred_genre = movies[pred_id].genre
        print(
